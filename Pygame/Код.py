...
background_1 = pygame.image.load(path.join(img_dir, "red.jpg"))
background_1 = pygame.transform.scale(background_1, (1400, 500))
...
def show_go_screen():
    ...
    draw_text(screen, "Платформер", 64, WIDTH // 2, HEIGHT // 4)
...
def show_win_screen():
    screen.blit(background, background_rect)
    draw_text(screen, f"Вы молодец! Ваш счет: {score}", 64, WIDTH // 2, HEIGHT // 4)
    draw_text(screen, "Перемещение - стрелки или 'A' и 'D'", 22, WIDTH // 2, HEIGHT // 2)
    draw_text(screen, "Прыжок - 'Пробел'", 22, WIDTH // 2, HEIGHT // 2 + 40)
    draw_text(screen, "Нажмите любую кнопку, чтобы попробовать снова!", 18, WIDTH // 2, HEIGHT * 3 // 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
def show_end_screen():
    ...
    draw_text(screen, f"Вы старались! Ваш счет: {score}", 64, WIDTH // 2, HEIGHT // 4)
    draw_text(screen, "Конец игры", 22, WIDTH // 2, HEIGHT // 2)
    draw_text(screen, "Нажмите любую кнопку, чтобы начать заново", 18, WIDTH // 2, HEIGHT * 3 // 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
while running:
    if game_over:
        ...
        platform4 = Block(WIDTH // 2 + 100, HEIGHT - 250, plat_img)
        all_sprites.add(platform4)
        platforms.add(platform4)
